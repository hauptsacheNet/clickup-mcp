 ...
 * A failure is logged and returns an empty array so one broken thread does not
 * take down the whole task view.
 */
export async function fetchCommentReplies(commentId: string): Promise<ExistingComment[]> {
  ...
  if (!response.ok) {
    console.error(
      `Error fetching replies for comment ${commentId}: ${response.status} ${response.statusText}`
    );
    return [];
  }
